...
def generate_keywords(query: str, openai_api_key: str):
    model = "gpt-3.5-turbo"
    max_tokens = 150
    ...
    prompt += '{ "industry_keywords": [], "location_keywords": [], "range_of_employees": []}'

    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are an expert in keyword extraction for finding relevant companies. Your task is to extract keywords from the user query to locate businesses in a large dataset."},
...
def fetch_leads(task):
    r = requests.post("https://api.apollo.io/v1/mixed_people/search", json=task)
...
def getting_cxo_result(content):
    ...
    organizations = []
    for account in org_response.get("accounts", []) + org_response.get("organizations", []):
        organization_data = {
    ...
    leads = []
    if total_orgs > 0:
        org_domains = "\n".join([org["domain"] for org in organizations if org["domain"] not in ("N/A", None)])
    ...
    for i in lead_response['contacts']:
        ...
        extracted_details.append({
            ...
            'company_domain': i.get('organization', {}).get('primary_domain', 'N/A'),
            'name': i.get('name', 'N/A'),
